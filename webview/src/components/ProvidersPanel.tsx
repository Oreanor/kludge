...
export default function ProvidersPanel({ providers, disabledProviders, onSave, onRemove, onRestore, onToggleProvider, t }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      {providers.map(p => {
        const enabled = !disabledProviders.includes(p.id)
        return (
          <div key={p.id} style={styles.toolRow}>
            <input
              ...
              style={{ cursor: 'pointer', flexShrink: 0, accentColor: 'var(--vscode-button-background)' }}
              title={enabled ? 'Disable models' : 'Enable models'}
            />
            <span style={{ flexShrink: 0, fontSize: 11, minWidth: 86, opacity: enabled ? 0.85 : 0.4 }}>{p.name}</span>
